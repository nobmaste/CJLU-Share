...
/* TIM1 init function */
void MX_TIM1_Init(uint16_t pre,uint16_t per)//倍频输出
{
  ...
  htim1.Instance = TIM1;
  htim1.Init.Prescaler = pre-1;
  htim1.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim1.Init.Period = per-1;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)// 捕获中断发生时执行
{
	...
	else if(IC_DONE_FLAG == 1)  // 如果完成一次高电平捕获
		{
			unsigned int time=0;
			IC_DONE_FLAG = 0;  // 标志清零
