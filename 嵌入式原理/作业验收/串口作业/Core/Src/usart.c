	...
	else
	{
		...
		if((RxBuffer[Uart1_Rx_Cnt-1] == 0x0A)&&(RxBuffer[Uart1_Rx_Cnt-2] == 0x0D)) //判断结束位
		{
		  ...
		  if((RxBuffer[Uart1_Rx_Cnt-3] == 0x12)&&(RxBuffer[Uart1_Rx_Cnt-6] == 0x12)){
				/*
				命令处理
				DATA_L=RxBuffer[Uart1_Rx_Cnt-4];增减CCRx值,十六进制输入
				DATA_H=RxBuffer[Uart1_Rx_Cnt-5];增减行为:0减小、1增加
				*/
				if(RxBuffer[Uart1_Rx_Cnt-5]==0){//减小亮度
					if((CCRx-10*RxBuffer[Uart1_Rx_Cnt-4])>=10 &&(CCRx-10*RxBuffer[Uart1_Rx_Cnt-4])<=90)	
						CCRx=CCRx-10*RxBuffer[Uart1_Rx_Cnt-4];					HAL_TIM_Base_Stop(&htim3);//停止定时器
					MX_TIM3_Init(CCRx);
					HAL_TIM_PWM_Start(&htim3,TIM_CHANNEL_1);
				}
				else if(RxBuffer[Uart1_Rx_Cnt-5]==1){//增加亮度
					if((CCRx+10*RxBuffer[Uart1_Rx_Cnt-4])>=10 &&(CCRx+10*RxBuffer[Uart1_Rx_Cnt-4])<=90)	
